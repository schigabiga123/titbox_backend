...
const MIGRATE_PROJECT_MAX_ATTEMPTS = 3
const MIGRATE_PROJECT_RETRY_DELAY_MS = 5000
const MIGRATE_PROJECT_TRANSACTION_TIMEOUT_MS = 120000
const MIGRATE_PROJECT_TRANSACTION_MAX_WAIT_MS = 20000
...
function isPrismaConnectionClosedError(error: unknown) {
  return typeof error === "object" && error !== null && "code" in error && (error as { code?: unknown }).code === "P1017"
}
...
async function migrateProject(
  ...
  const projectTitle = asString(projectData.title) ?? projectDoc.id
  ...
  await prisma.$transaction(
    async (tx) => {
      ...
      if (existingProject) {
        console.log(`Skipping project ${projectIndex}/${totalProjects}: title already exists in database. title=${existingProject.title}, projectId=${existingProject.id}`)
        return
      }
      ...
      for (const [taskIndex, taskItem] of tasks.entries()) {
        ...
        for (const field of taskFields) {
          const createdField = await tx.taskField.create({
            data: {
              taskId: field.taskId,
        ...
        const shouldCreateDefaultPlombaField = createdTask.isPickUp === false && !taskFields.some((field) => field.name === "plomba")

        if (shouldCreateDefaultPlombaField) {
          await tx.taskField.create({
            data: {
        ...
        if (taskComments.length > 0) {
          console.log(`[project ${projectIndex}/${totalProjects}][task ${taskIndex + 1}/${totalTasks}] Project with comments migrated: projectId=${createdProject.id}, taskId=${createdTask.id}, comments=${taskComments.length}`)
        }
        ...
        if (Object.keys(portaChecklist).length > 0) {
          if (hasPortaChecklist) {
            console.log(`[project ${projectIndex}/${totalProjects}][task ${taskIndex + 1}/${totalTasks}] Project with porta checklist migrated: projectId=${createdProject.id}, taskId=${createdTask.id}`)
          }

          await tx.portaChecklist.create({
            data: {
              taskId: createdTask.id,
        ...
        if (Object.keys(libra).length > 0) {
          if (hasLibra) {
            console.log(`[project ${projectIndex}/${totalProjects}][task ${taskIndex + 1}/${totalTasks}] Project with libra migrated: projectId=${createdProject.id}, taskId=${createdTask.id}`)
          }

          const taskFieldId = asString(libra.taskFieldId)
          const libraPlomba = asRecord(libra.plomba)
          let resolvedLibraTaskFieldId = taskFieldId ? (taskFieldIdMap.get(taskFieldId) ?? null) : null

          if (Object.keys(libraPlomba).length > 0) {
            const createdPlombaField = await tx.taskField.create({
              data: {
                taskId: createdTask.id,
                name: "plombaLibra",
                type: "text",
    ...
    {
      maxWait: MIGRATE_PROJECT_TRANSACTION_MAX_WAIT_MS,
      timeout: MIGRATE_PROJECT_TRANSACTION_TIMEOUT_MS,
    },
  )
}

async function migrateProjectWithRetry(
  projectDoc: admin.firestore.QueryDocumentSnapshot,
  projectIndex: number,
  totalProjects: number
) {
  for (let attempt = 1; attempt <= MIGRATE_PROJECT_MAX_ATTEMPTS; attempt += 1) {
    try {
      await migrateProject(projectDoc, projectIndex, totalProjects)
      return
    } catch (error) {
      if (!isPrismaConnectionClosedError(error) || attempt === MIGRATE_PROJECT_MAX_ATTEMPTS) {
        throw error
      }

      console.warn(`Project migration connection closed. Retrying project ${projectIndex}/${totalProjects}, attempt ${attempt + 1}/${MIGRATE_PROJECT_MAX_ATTEMPTS}.`)
      await prisma.$disconnect().catch(() => undefined)
      await delay(MIGRATE_PROJECT_RETRY_DELAY_MS * attempt)
...
async function migrateProjects() {
  ...
  for (const [projectIndex, projectDoc] of snapshot.docs.entries()) {
    ...
    if(true){ //i<500 //title == 'SZF-26-0005121'
      await migrateProjectWithRetry(projectDoc, projectIndex + 1, snapshot.docs.length)
      i++;
    }
  }
